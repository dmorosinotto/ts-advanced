...
const StoryV = t.intersection(
  [
    t.type({
      ...
      text: t.optional(t.string), // HTML content if story is a text post
      url: t.optional(t.string) // URL of linked article if the story is not text post
    }),
    ItemCommonV,
    TopLevelV
  ],
...
function decodeToPromise<T, O, I>(
  ...
  const  jsToString = function (value: any) { return (value === undefined ? 'undefined' : JSON.stringify(value)); };
  const formatValidationError = function (error: t.ValidationError ) {
